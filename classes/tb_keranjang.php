...
class Keranjang {
    ...
    public function addToCart($user_id, $produk_id, $harga, $jumlah) {
        $sql = "INSERT INTO tb_keranjang (ker_id_user, ker_id_produk, ker_harga, ker_jml) VALUES (?, ?, ?, ?)";
        $this->db->executeQuery($sql, [$user_id, $produk_id, $harga, $jumlah]);
    }

    // Method to update an item in the cart
    public function updateCartItem($cart_id, $jumlah) {
        $sql = "UPDATE tb_keranjang SET ker_jml = ? WHERE ker_id = ?";
        $this->db->executeQuery($sql, [$jumlah, $cart_id]);
    }
